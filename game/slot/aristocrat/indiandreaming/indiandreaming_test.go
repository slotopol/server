...
func TestScanner(t *testing.T) {
	...
	for i, p := range presets {
		...
		g.Scanner(&wins)
		if len(wins) != p.Num {
			t.Errorf("error at %d grid on regular spins, expected %d, gets %d wins", i+1, p.Num, len(wins))
		}
		gain = wins.Gain()
		if gain != p.WinReg {
		...
		gain = wins.Gain()
		if len(wins) != p.Num {
			t.Errorf("error at %d grid on free spins, expected %d, gets %d wins", i+1, p.Num, len(wins))
		}
		if gain != p.WinBon {
