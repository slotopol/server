...
func CalcStat(ctx context.Context, mrtp float64) float64 {
	...
	var calc = func(w io.Writer) float64 {
		...
		var qbb = s.BonCountF(bbid) / reshuf
		var rtpbb = Ebb * qbb * 100
		var qls1 = s.BonCountF(lsb1) / reshuf / float64(g.Sel)
		var rtpls1 = Els * 1 * qls1 * 100
		var qls3 = s.BonCountF(lsb3) / reshuf / float64(g.Sel)
		var rtpls3 = Els * 3 * qls3 * 100
		var qls6 = s.BonCountF(lsb6) / reshuf / float64(g.Sel)
