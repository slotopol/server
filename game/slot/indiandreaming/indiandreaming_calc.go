...
func CalcStatBon(ctx context.Context, rn string) float64 {
	...
	var dur = func() time.Duration {
		...
		defer cancel2()
		go s.Progress(ctx2, time.Tick(10*time.Second), sln, float64(s.Planned()))
		slot.BruteForce5xGo(ctx2, &s, g, reels)
		//slot.MonteCarloGo(ctx2, &s, g, reels)
...
func CalcStatReg(ctx context.Context, rn string) float64 {
	...
	var planned = reels.Reshuffles()
	//var total = 10e6
	var dur = func() time.Duration {
		var t0 = time.Now()
		var ctx2, cancel2 = context.WithCancel(ctx)
		defer cancel2()
		go s.Progress(ctx2, time.Tick(10*time.Second), sln, float64(planned))
		slot.BruteForce5xGo(ctx2, &s, g, reels)
		//slot.MonteCarloGo(ctx2, &s, g, reels, planned)
		return time.Since(t0)
	}()
	...
	fmt.Printf("completed %.5g%%, selected %d lines, time spent %v\n", reshuf/float64(planned)*100, g.Sel.Num(), dur)
	fmt.Printf("reels lengths [%d, %d, %d, %d, %d], total reshuffles %d\n",
		len(reels.Reel(1)), len(reels.Reel(2)), len(reels.Reel(3)), len(reels.Reel(4)), len(reels.Reel(5)), reels.Reshuffles())
	fmt.Printf("symbols: %.5g(lined) + %.5g(scatter) = %.6f%%\n", lrtp, srtp, rtpsym)
	fmt.Printf("free spins %d, q = %.5g, sq = 1/(1-q) = %.6f\n", s.FreeCount, q, sq)
