...
func CalcStat(ctx context.Context, sp *slot.ScanPar) (float64, float64) {
	...
	var sb = slot.NewStatGeneric(sn, 4)
	{
		var reels = ReelsBon
		var g = NewGame(sp.Sel)
		g.FSR = 12 // set free spins mode
		var calc = func(w io.Writer) (float64, float64) {
			return slot.Parsheet_generic_fgretrig(w, sp, sb, g.Cost(), 1, 12)
		}
	...
	var sr = slot.NewStatGeneric(sn, 4)
	{
		var reels, _ = ReelsMap.FindClosest(sp.MRTP)
		var g = NewGame(sp.Sel)
		var calc = func(w io.Writer) (float64, float64) {
			return slot.Parsheet_generic_fgretrig_split(w, sp, sr, sb, g.Cost(), 1, 12)
		}
